...
//            /register login nick password
            ...
            switch (command) {
                ...
                case "/register": {
                    ...
                    boolean isRegistred = server.getAuthenticationProvider().register(login, password, nick, isAdmin);
                    if (!isRegistred) {
    ...
    private void communicateWithUser(Server server) throws IOException {
        while (true) {
            ...
            if (message.startsWith("/")) {
                if (message.equals(Commands.EXIT.toString())) {
                    break;
                } else if (message.equals(Commands.LIST.toString())) {
                    List<String> userList = server.getUserList();
                    String joinedUsers =
                            String.join(", ", userList);
//                            userList.stream().collect(Collectors.joining(","));
                    sendMessage(joinedUsers);
                } else if (message.startsWith(Commands.WHISPER.toString())) {

                    server.whisper(parseCommandName(message)[0], parseCommandName(message)[1]);
                } else if (message.startsWith(Commands.KICK.toString()) && server.getAuthenticationProvider().isAdmin(username)) {
                    server.kick(parseCommandName(message)[0]);
                }
            } else {
